...
def kernRR( kern1, kern0, dx=1, dy=1, row=1, col=1):
    return math.ceil( float(kern1) / kern0 ) #* \
...
def fmRR(fm1, fm0):
    return math.ceil(fm1 / float(fm0))
    
def rowcolBuff(kern, fm, dx, dy):
    return kern * fm * dx * dy
    
def rowRR(row1, row0):
    return math.ceil(row1 / float(row0)) 
    
def colRR(col1, col0):
    return math.ceil(col1 / float(col0)) 
